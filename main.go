...
func processInfix(LL int){
	...
	for i := 0; i<LL; {
		if infix[i] < 0 {
			if infix[i] == mul && infix[i-1]>=0 && infix[i+1]>=0 && mark[i-1] == 0 {
				...
				mark[i+1] = 1
				i += 2
			}else if infix[i] == divide && infix[i-1]>=0 && infix[i+1]>=0 && mark[i-1] == 0 {
				preInfix[index-1] = brakcetOpen
				preInfix[index] = infix[i-1]
				index++
				...
				index++	
				i += 2
			}else if infix[i] == minus && infix[i-1]>=0 && infix[i+1]>=0 && mark[i-1] == 0 {
				preInfix[index-1] = brakcetOpen
				preInfix[index] = infix[i-1]
				index++
				preInfix[index] = infix[i]
